...
Tibble columns are vectors, so `map_chr()` treats `feeling` just like any other vector.

```{r echo=FALSE}
knitr::include_graphics("images/map-mutate-emoticons-1.png", dpi = image_dpi)
```

`map_chr()` then applies `emoticons_1()` to each element of `feelings`,

```{r echo=FALSE}
knitr::include_graphics("images/map-mutate-emoticons-2.png", dpi = image_dpi)
```

producing a vector with an emoticon for each element of `feelings`.

```{r echo=FALSE}
knitr::include_graphics("images/map-mutate-emoticons-3.png", dpi = image_dpi )
```
