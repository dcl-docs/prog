...
Supply `unnest()`'s `cols` argument with the name of the columns to unnest.
...
```{r}
summarize(., prop = sum(str_detect(name, "a$")) / n()) %>% 
  pull(prop)
```
...
Note that `.` does not like to be piped into functions, so we couldn't pipe `.` into `summarize()`. 
