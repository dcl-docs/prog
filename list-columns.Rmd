...
Pass `nest()` the name of the column to put into each individual tibble. `nest()` will create one row for each unique value of the remaining variables. For example, say we select just two columns from `countries`
...
You're used `group_by()` and `summarize()` collapsing groups into single rows. We can also use `summarize()` to create a list column, where each element is a vector, list, or tibble. 
...
Here's another example, which only stores countries that begin with "A" in `countries`.
...
The third way to create a list column involves functions that take a vector as input and return a list. `map()` is one such function.  

In the following code, `map()` iterates over `name`, generating a vector of random numbers for each country. 

```{r}
countries %>% 
  select(name) %>% 
  mutate(random = map(name, ~ rnorm(n = str_length(.))))
```
...
Supply `unnest()`'s `col` argument with the name of the column to unnest.
...
To manipulate list columns, you'll need to use purrr. For example, say we want to find the number of countries in each continent. Here's `regions` again.
...
Instead, we need to iterate over `countries`, taking the length of each element separately.

```{r}
regions %>% 
  mutate(num_countries = map_int(countries, nrow))
```

Note that we need to use `nrow()` because each element of `countries` is actually a tibble. 

When manipulating list columns, it's often helpful to use the `.` strategy discussed in [Basic map functions](https://dcl-prog.stanford.edu/purrr-basics.html#anonymous-functions-1). Assign the first element of the column to `.`.
...
Then, test out your manipulation on `.`. For example, the following finds the proportion of country names that end in "a."

```{r}
count(., ends_in_a = str_detect(name, "a$")) %>% 
  mutate(prop = n / sum(n)) %>% 
  filter(ends_in_a) %>% 
  pull(prop)
```
...
Note that `.` does not like to piped into functions, so we couldn't pipe `.` into `count()`. 
