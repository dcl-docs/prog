...
You'll spend most of your time in R working with three different data structures: atomic vectors, lists, and tibbles. In this reading, you'll learn the differences between the three and how to manipulate each one.

In R, almost every data structure you'll work with is a vector. In this book, we'll often represent vectors like this:
...
_Atomic vectors_ are one of two kinds of vectors. _Lists_, as you'll see later on, are the other kind.

```{r echo=FALSE}
knitr::include_graphics(
  "images/data-structure-basics/vector-atomic-list.png", 
  dpi = image_dpi
)
```
...
Integer atomic vectors contain only integers, double atomic vectors contain only doubles, and so on. Together, integer and double vectors are known as __numeric__ vectors. All vectors can also contain the missing value `NA`. 

In R, single numbers, logicals, and strings are just atomic vectors of length 1, which means when you run

```{r}
x <- "a"
```

you're creating a character vector. Likewise, 
...
Vectors (both atomic vectors and lists) all have two key properties: a __type__ and a __length__.
...
We can _subset_ `v` to select specific elements, ignoring the others. The operators `[` and `[[` subset vectors. Use `[` to select multiple elements, and `[[` to select just one. We'll cover four ways to use `[` and then `discuss `[[`. 
...
Subset with a vector of negative integers to _exclude_ elements. The following code removes the first and third elements of `v`.
...
If a vector has names, you can also subset with a character vector of names. 
...
If you supply a vector of `TRUE`s and `FALSE`s to `[`, it will select the elements that correspond to the `TRUE`s. 
...
You'll rarely pass a vector of `TRUE`s and `FALSE`s that you created by hand to `[`. Instead, it's much more common to create a logical vector with a function or condition. 
...
`[` then uses this logical vector to subsets `v`, resulting in just the elements of `v` greater than 2. 

Say we have a vector with `NA`s.
...
Vectors are central to programming in R, and so many (if not most) R functions will work with vectors of any length.
...
Tibbles are a specific kind of list. 
...
Every tibble is a list of vectors. Each tibble column is its own vector. 
