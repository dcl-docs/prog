...
`map_dfr()` is useful when you want to read in data from multiple files. The following code reads in several very simple csv files that contain the names of different dinosaur genera.
...
This is useful when working with tibbles. You can use `keep()` and `discard()` to select columns that meet a certain condition.
